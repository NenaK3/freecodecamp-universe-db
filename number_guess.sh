...
do
  ...
  then
    ...
    echo "$USER_STATS" | while IFS="|" read G_PLAYED B_GAME
    do
      NEW_GAMES_PLAYED=$(( G_PLAYED + 1 ))
      
      if [[ -z $B_GAME || $GUESS_COUNT -lt $B_GAME ]]
      then
        UPDATE_RES=$($PSQL "UPDATE users SET games_played=$NEW_GAMES_PLAYED, best_game=$GUESS_COUNT WHERE username='$USERNAME'")
      else
